<template>
    <v-card class="bg-grey-lighten-3 pa-4 fill-height" height="500">
        <div class="d-flex justify-space-between text-grey-darken-4 text-subtitle-2">
            <div>{{ props.label }}</div>
            <div>{{ Math.round(props.percentage) }}%</div>
        </div>
        <div class="py-3">
            <v-progress-linear v-model="props.percentage" height="9" :color="percentageColor" rounded buffer-color="#6a3e0b"
      buffer-opacity="1"
      :buffer-value="totalValue" class="auto-draw"></v-progress-linear>
        </div>
